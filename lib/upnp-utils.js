...
class UPnPUtils extends mEventEmitter {
    ...
    *          |         |          | The value must be in the range of 1 to 60.
    ...
    * -------------------------------------------------------------------------- */
    async discover(params = {}) {
        ...
        if (typeof (sec) !== 'number' || sec % 1 !== 0 || sec < 1 || sec > 60) {
            throw new Error('The `wait` must be an itenger in the range of 1 to 60.');
        }
    ...
    * -------------------------------------------------------------------------- */
    ...
    async _receivePacket(buffer, rinfo) {
        ...
        if (headers['$'].match(/^HTTP\/[\d\.]+\s+200\s+OK/)) {
            if (!this._devices[usn]) {
                ...
                try {
                    const { dheaders, xml, obj } = await this._fetchDeviceDescriptions(loc);
    ...
    async _fetchDeviceDescriptions(url) {
        let dd = null;
        let err = null;
        for (let i = 0; i < 3; i++) {
            try {
                dd = await this._fetchXml(url);
                err = null;
                break;
            } catch (error) {
                err = error;
        ...
        return { dheaders: dd.dheaders, xml: dd.xml, obj: obj };
    }
    ...
    _fetchXml(url) {
        return new Promise((resolve, reject) => {
            ...
            mHttp.get(url, opts, (res) => {
                const dheaders = res.headers;
                const code = res.statusCode;
                if (code !== 200) {
                    res.resume();
                    const msg = `HTTP RESPONSE ERROR: url=${url}, statusCode=${code}`;
